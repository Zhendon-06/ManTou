...
2. **必须返回 JSON 字符串**：统一 schema `{"success": bool, "data": any, "error": string|null}`。
3. **必须有 @MantouTool 类注解**：name 必须唯一，是 JS 方法名的前缀（如 `alarm` → `alarmSet`）。
4. **必须有 @ToolMethod 和 @ToolReturns 方法注解**：生成 API 文档时用。
5. **参数必须有 @ToolParam 注解**：指定公开 name 和用途，供 KSP 生成文档。
6. **普通 Tool 由 KSP 自动注册**：只有依赖页面上下文的 Tool 才设置 `autoRegister = false` 并由宿主单独注入。
7. **不做耗时操作**：不要假设 JSBridge 在主线程；UI 操作切主线程，长任务使用异步回调。
8. **权限敏感操作必须先检查**：没有权限时返回 error JSON，网页侧提示用户去设置。

### 6.2 LLM Prompt 规则

1. **路由层只做路由**：不输出业务内容，只输出 JSON。
2. **web_app 分支必须注入 tool API 文档**：LLM 必须知道能调什么（文档由 `tool-ksp` 编译期生成）。
...
6. **方法签名**: 参数用基本类型，返回 `String`（JSON）
7. **返回值 schema**: `{"success": bool, "data": any, "error": string|null}`
8. **运行 KSP 校验**: `./gradlew :app:kspDebugKotlin`
...
| 3 | `tool/ToolRegistry.kt` | ✅ 已创建 | 缓存 KSP 生成的 Tool 实例 |
...
| 7.1 | `tool-ksp` | ✅ 已创建 | KSP 校验 Tool 契约并生成文档与注册表 |
| 7.2 | `build/generated/ksp/<variant>/resources/mantou_tools.md` | ✅ 自动生成 | 可检查和归档的 Android 能力文档 |
...
| 12 | `utils/AppGenerator.kt` | ✅ 已扩展 | `buildSystemPrompt` 拼接 KSP 生成文档；`WEB_APP_RUNTIME_GUARD` 增加 JS 别名表逻辑，把 `MantouApp_<name>` 挂到 `MantouApp.<name>` |
